...
func (s *Server) prepareBrokerMsg() {
	...
	s.brokerMessage = writeInt32(s.brokerMessage, portN)
}
...
func (s *Server) handle(c net.Conn) error {
	...
	case 3: // Metadata
		...
		response = writeArrayHeader(response, len(topics))
...
func (s *Server) readProduceRequest(request []byte) (topic string, partition, offset int) {
...
func writeArray(dst []byte, data [][]byte) []byte {
	dst = writeArrayHeader(dst, len(data))
	for _, d := range data {
		dst = append(dst, d...)
	}
	return dst
